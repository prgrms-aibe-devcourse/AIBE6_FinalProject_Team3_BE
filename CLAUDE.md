...
- `com.algogyeyak.auth.token` — `RefreshTokenService`: **Redis-backed since 2026-08-03** (previously DB — see `docs/specs/auth-design.md` for the reversal rationale), single session per user — a new login or refresh immediately invalidates the previous raw token. Two Redis keys per session (`by-hash:{tokenHash}`→userId, `by-user:{userId}`→tokenHash reverse index), both TTL'd to the refresh token validity so Redis handles natural expiry — no more `expiresAt` column/manual expiry check, and `AUTH_REFRESH_TOKEN_EXPIRED` is no longer thrown (collapsed into `AUTH_REFRESH_TOKEN_INVALID`, which the frontend already treated the same). Rotation/revocation use `ValueOperations.getAndDelete` for atomic single-use consumption (replaces the old JPA `PESSIMISTIC_WRITE` lock). Raw tokens are never stored, only a SHA-256 hash. Redis failures are fail-closed (`AUTH_TOKEN_STORE_UNAVAILABLE`, 503).
