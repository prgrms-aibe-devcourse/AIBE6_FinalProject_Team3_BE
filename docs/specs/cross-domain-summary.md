...
| user | 1개 (**(2026-08-24 갱신)** 8개 중 7개 해결 — 탈퇴 관련 4건 + 프로필 등록 409 정정 1건 + FE/BE 책임 소재 확인 2건(생성형 AI 사전고지·`currentStage` 위젯 우선순위, 둘 다 프론트 책임/필드 제거로 정리). 남은 1개는 S3 이중실패 엣지케이스뿐 — `user-design.md` 남은 이슈 8번 참고, 완전히 닫을 수 있는 방법은 확인했으나 구현 여부는 미정). **(2026-08-14 추가)** 같은 문서의 전수조사 결과 섹션에서 별도로 발견된 이슈(`interestRegion` 길이 검증 미비, 권한/상태변경-탈퇴 TOCTOU)는 해결/부분 해결됨 |
| property | 17개 |
| market-data | 4개 |
