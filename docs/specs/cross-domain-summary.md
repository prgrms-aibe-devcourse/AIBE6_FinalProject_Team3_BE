...
| `checklist` | 거의 완전 구현 | 요구사항 대비 확인 필요 8개 항목 전부 처리 완료(코드 5건 수정 + 문서화 3건 + 노션 명세서 반영). **(2026-08-06 추가)** `GET /checklists`(내 체크리스트 목록)에 DB 레벨 페이지네이션 추가 — 매물 목록(`GET /properties`)과 동일한 `PageResponse` 봉투 구조로 응답이 바뀜(기존 배열 응답에서 breaking change). **(2026-08-14 추가)** 실사용 피드백 반영 — 신규 문항 6개(단창/이중창·누전·차단기·보일러종류·냉난방방식·방범창), `ChecklistItemType.MULTIPLE_CHOICE` 신규 도입, 예시 이미지 스키마(`ChecklistItemTemplateImage`, `S3ImagePurpose.CHECKLIST_TEMPLATE`) 신설. 매물유형별 적용 문항 수 상한이 24개→30개로 늘어나 요구사항 범위 자체를 갱신함. **(2026-08-14 오후 추가)** 이미지 관리자 API(목록조회/추가/삭제) 구현 완료 — 재배포 없이 이미지 추가/삭제 가능(파일 업로드는 미지원, URL만 입력). **(2026-08-14 오후, HOTFIX)** 시더(`ChecklistTemplateSeeder`)가 이미 시딩된 DB엔 신규 문항을 반영 못하던 버그 발견·수정 — v3 신규 6개 문항이 실제 운영 DB에는 반영 안 되고 있었음. `content` 기준 없는 문항만 추가 + 기존 문항 재동기화 방식으로 변경. **(2026-08-18 추가)** `GET /checklists` 응답에 `progressPercent`/`cautionCount` 추가 — 매물 목록엔 이미 있던 진행률 표시가 체크리스트 목록엔 없어 비일관적이던 걸 보완. 매물 목록이 이미 쓰던 집계 쿼리(`ChecklistItemRepository.findProgressByUserId()`)를 그대로 재사용해 issueCount만 추가하는 식으로 확장, 새 쿼리 없이 항상 쿼리 2개로 끝나 N+1 없음(`Checklist.items`가 LAZY라 `computeResult()`를 그대로 썼다면 N+1이 생겼을 지점). **(2026-08-19 완료)** MULTIPLE_CHOICE 문항에서 "미흡" 선택 시 주의 항목(`issueFound`)으로 자동판정되지 않던 문제 해결 — `answerMultipleChoice()`가 값이 "미흡"이면 `issueFound=true`로 처리하도록 수정. 채광/수압/냉난방시설을 CHECK→MULTIPLE_CHOICE로 전환할 예정이라(FE mock 반영 완료, 관리자 페이지 템플릿 적용 예정) 전환 전에 미리 반영함. 거래유형별 분기 미도입만 열려있는 사항으로 남음 |
| `user` | 거의 완전 구현 | **(2026-08-11 갱신)** 이미지 업로드(S3 presign/confirm/reset), 탈퇴 처리(익명화 + 연관 데이터 정리 + 세션 무효화, FE 연결까지 완료)가 모두 끝났습니다. ~~남은 건 "생성형 AI 사전고지"가 FE 책임인지 확인하는 1건과, `currentStage` 기반 위젯 우선순위가 FE 책임인지 확인하는 1건~~ **(2026-08-24 해결)** 둘 다 정리됨 — 생성형 AI 사전고지는 프론트 책임으로 확정, `currentStage`(자취 단계) 필드는 백엔드에서 완전히 제거됨(`ProfileRegisterRequest`/`ProfileUpdateRequest`/`UserProfileResponse`/`UserPreference`/`CurrentStage` enum, 커밋 `2bed1d9`). ~~남은 건 S3 이전 이미지 정리(태깅+즉시삭제)가 둘 다 실패하는 낮은 확률의 엣지케이스 1건뿐~~ **(2026-08-24 해결)** `property`의 `PropertyImageOrphanCleanupJob`을 그대로 미러링한 `ProfileImageOrphanCleanupJob`을 신설해 완전히 닫음(`user-design.md` 남은 이슈 8번 참고). **(2026-08-14 추가)** `user-design.md` 전수조사 결과에서 새로 발견된 이슈 2건도 처리됨 — `interestRegion` 길이 미검증(해결, `@Size(max=30)`) / 관리자 권한·상태 변경이 본인 탈퇴와 경쟁하는 TOCTOU(부분 해결, `AdminUserService`를 조건부 UPDATE로 교체 — 완전 봉쇄는 아직 아님). 자세한 내용은 `user-design.md`/`admin-design.md` 전수조사 결과 참고 |
...
| `risk-analysis` | 완전 구현 | **(2026-08-04 완료)** 신호 탐지기 4종, API 4개(`POST /risk-analysis`, `GET /risk-signals`, `GET /deposit-safety`, `POST /deposit-safety/recalculate`), market-data 어댑터(전세+매매), `DepositSafetyCheckService`(전세가율), checklist 연계 보조 신호, 매물 수정 시 자동 재계산 트리거(이벤트 기반)까지 전부 구현 완료. ~~완전 미해결 이슈 없음 — 남은 건 "동일 계정 다수 등록" 활성화 여부·선순위보증금 입력 화면 배치 등 팀/FE 결정 2건뿐~~ — **(2026-08-12 정정)** 요구사항 기능 자체는 전부 구현됐지만, 같은 날 진행된 코드 전수조사에서 새로운 버그/코드품질 이슈 5건이 발견됨(트랜잭션 경계 불일치, 판정불가 사유 오분류, 죽은 설정값, 역방향 의존, `@Version` 부재) — `risk-analysis-design.md` 전수조사 결과 섹션 참고, 팀/FE 결정 대기 2건과는 별개. ✅ **(2026-08-18 전부 해결)** 5건 모두 코드 수정 완료 — 트랜잭션 경계 불일치는 fail-safe 흡수, 사유코드는 거래유형/매물유형 분리, 죽은 설정값은 제거, `PropertyService`↔risk-analysis 역방향 의존은 `PropertyRiskSummaryProvider` 어댑터로 정리, 3개 엔티티에 `@Version` 추가. ✅ **(2026-08-19 정정)** "팀/FE 결정 대기 2건" 중 선순위보증금 입력 화면 배치는 이미 FE에 구현돼 있던 걸 뒤늦게 확인함(`RiskAnalysisClient.tsx`) — 실제 남은 결정은 "동일 계정 다수 등록" 활성화 여부 1건뿐. **(2026-08-19 완료)** 신호 탐지기 중 `DuplicateListingDetector`/`ShortTermRelistingDetector` 2개가 원래 공개 매물 플랫폼(임대인 어뷰징 탐지)을 전제로 설계돼 있어 세입자가 개인 계정에 비공개로 매물을 등록하는 이 서비스 구조와 안 맞던 문제 발견·수정 — "의심된다"는 시스템 판정을 없애고 비교 가능한 사실(가격·등록 시점)만 담은 문장으로 톤 변경. FE 변경 없음(`description`이 원래도 자유 텍스트). `SameAccountMultipleDetector`도 같은 종류의 전제 불일치가 있으나 이미 비활성화 상태라 이번엔 admin 담당자 논의로 넘김(`risk-analysis-design.md` 참고). |
...
| user | 0개 (**(2026-08-24 갱신)** 8개 전부 해결 — 탈퇴 관련 4건 + 프로필 등록 409 정정 1건 + FE/BE 책임 소재 확인 2건(생성형 AI 사전고지·`currentStage` 위젯 우선순위, 둘 다 프론트 책임/필드 제거로 정리) + S3 이중실패 엣지케이스 1건(`ProfileImageOrphanCleanupJob` 신설로 해결) — `user-design.md` 참고). **(2026-08-14 추가)** 같은 문서의 전수조사 결과 섹션에서 별도로 발견된 이슈(`interestRegion` 길이 검증 미비, 권한/상태변경-탈퇴 TOCTOU)는 해결/부분 해결됨 |
| property | 17개 |
| market-data | 4개 |
...
| risk-analysis | ~~0개 (2026-08-04 완료, 원본 9개 전부 해결 + 신규 발견 1개도 해결. 남은 건 팀/FE 결정 대기 2건뿐)~~ — **(2026-08-12 정정)** 이 개수는 요구사항 대비 "확인 필요" 항목 기준이라 여전히 유효하나, 같은 날 진행된 코드 전수조사(품질/버그 관점)에서 별도로 5건이 새로 발견됨 — `risk-analysis-design.md` 전수조사 결과 섹션 참고. ✅ **(2026-08-18 정정)** 이 5건도 전부 해결 완료 — 요구사항 기준·전수조사 기준 모두 열려있는 항목 0개 |
