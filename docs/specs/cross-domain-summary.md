...
- `S3KeyGenerator.isPropertyImageOwnedBy(Long userId, String key)`도 `isProfileImageOwnedBy`와 동일한 패턴으로 추가했지만, **아직 아무도 호출하지 않는다** — `PropertyImageUploadController`는 이번 작업 범위(공통 코드만) 밖이라 건드리지 않음. **주의**: 매물 이미지 key는 propertyId가 아니라 userId 기준으로 생성되므로(`PropertyImageUploadController` 클래스 javadoc, `propertyImageKey()` 참고) 파라미터명도 `userId`로 맞췄다 — 처음엔 `propertyId`로 잘못 이름 붙였다가 리뷰에서 지적받아 정정함(2026-08-12).
- **여전히 남은 문제**: 위 목록의 "**로그인한 사용자 B가 다른 사용자 A의 pending property-images/{A}/... key를 confirm**" 케이스(같은 purpose, 다른 소유자)는 이번 수정으로 막히지 않는다 — prefix 검증은 "이 key가 이 purpose에 속하는가"만 볼 뿐 "이 key가 이 *호출자*의 것인가"는 판단할 수 없고, 후자는 호출자의 userId를 아는 컨트롤러 레벨에서만 가능하다. `PropertyImageUploadController.confirm()`이 `@AuthenticationPrincipal` + 위에서 준비해둔 `isPropertyImageOwnedBy()`를 호출하도록 고치는 후속 작업이 property 담당자에게 필요하다.
...
1. **`.env.example`에 `KAKAO_REST_API_KEY`와 `MOLIT_SERVICE_KEY`가 두 번씩 선언돼 있다** — 카카오 로컬 API 설명 주석 블록(`.env.example:39`, `:43`)에 한 번씩 정의된 뒤, 파일 뒤쪽 AWS/DB 섹션 바로 앞(`:55-56`)에 주석 없이 다시 나열되어 있다. 이 파일을 그대로 `backend/.env`로 복사해 쓰는 게 문서화된 표준 워크플로우(`CURRENT_STATE.md` "소셜 로그인 / 환경변수" 절)인데, 중복된 키가 있으면 실제로 값을 채워야 하는 곳이 어디인지 헷갈리기 쉽고, 어느 값이 최종적으로 적용되는지도 이 파일만 보면 알 수 없다. 여러 번에 걸쳐 섹션이 추가되면서 생긴 병합 실수로 보인다.

2. **`docker-compose.monitoring.override.yml`은 이름은 `docker-compose.override.yml`과 비슷하지만 자동으로 병합되지 않는다.** Docker Compose CLI가 `-f` 없이도 자동으로 얹어주는 override 파일명은 정확히 `docker-compose.override.yml`뿐이다. `docker-compose.monitoring.yml`을 로컬에서 테스트하려는 사람은 `-f docker-compose.monitoring.yml -f docker-compose.monitoring.override.yml`을 매번 직접 지정해야 하는데, 이 조합을 안내하는 문서(README.md/CURRENT_STATE.md)나 스크립트가 저장소에 없다. `docker-compose.override.yml`(로컬 backend/redis용)은 파일명 규칙 덕분에 암묵적으로 동작하지만 monitoring 쪽은 그렇지 않다는 비대칭이 있어, 실제로 로컬에서 모니터링 스택을 띄워보려는 팀원이 빠뜨리기 쉬운 지점이다.
...
8. **checklist 관리자 템플릿 버전 산정이 "비활성 문항에 더 높은 버전이 남아있는" 경우를 테스트하지 않는다.** `AdminChecklistTemplateServiceTest.java`(`backend/src/test/java/com/algogyeyak/checklist/service/AdminChecklistTemplateServiceTest.java`)의 `template()` 헬퍼(L44-56)는 `.active(true)`를 무조건 고정해서 만든다. `createInheritsHighestExistingVersion()`(L89-108)도 두 템플릿 다 활성 상태로만 구성해 "가장 높은 버전을 물려받는다"를 검증하는데, 이건 활성 템플릿만 있을 때는 우연히 맞는 값이다. 정작 문서가 지적한 위험 시나리오(비활성 템플릿 중 하나가 활성 템플릿들보다 더 높은 버전을 가진 경우 새 문항이 실제 활성 배치보다 앞서가는 버전을 받는 문제)를 재현하는 테스트는 없다 — `template(id, version, order)`를 `.active(false)`로도 만들 수 있는 오버로드조차 없다.
9. **auth `LocalAuthService.login()`의 SUSPENDED 계정 처리(계정 없음과 동일하게 뭉뚱그리는 부분)가 이 도메인 테스트에서 전혀 검증되지 않는다.** `LocalAuthServiceTest.java`(`backend/src/test/java/com/algogyeyak/auth/service/LocalAuthServiceTest.java`) 전체를 봐도 "suspend"/"SUSPENDED"를 다루는 테스트 메서드가 없다(반면 `JwtAuthenticationFilterTest.doesNotSetAuthenticationWhenUserHasBeenSuspendedSinceTokenWasIssued`와 `AdminUserServiceTest`/`AdminUserControllerTest`는 각자의 관점에서 SUSPENDED를 다룬다). 문서가 지적한 "로그인 경로마다 계정 열거 방지 수준이 다르다"는 문제를 검증하려면 최소한 로컬 로그인이 SUSPENDED 계정에 대해 실제로 `AUTH_INVALID_CREDENTIALS`(계정 없음과 동일)를 반환하는지를 확인하는 테스트가 있어야 하는데, 그 자체가 없어 이 경로의 현재 동작이 테스트로 고정(pin)되어 있지 않다 — 향후 실수로 이 경로가 SUSPENDED 여부를 노출하는 방향으로 바뀌어도 잡아낼 회귀 테스트가 없다.
