...
`property`와 `checklist`는 이제 같은 방향(명확히 구분)으로 맞춰졌지만, `user`는 여전히 다른 방식이라 완전한 팀 공통 정책은 아직 없습니다.
