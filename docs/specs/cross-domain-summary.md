...
- `S3KeyGenerator.isPropertyImageOwnedBy(Long propertyId, String key)`도 `isProfileImageOwnedBy`와 동일한 패턴으로 추가했지만, **아직 아무도 호출하지 않는다** — `PropertyImageUploadController`는 이번 작업 범위(공통 코드만) 밖이라 건드리지 않음
