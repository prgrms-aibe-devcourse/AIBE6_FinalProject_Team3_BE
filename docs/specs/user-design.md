...
| 길이/형식 검증 | **(2026-08-11)** 닉네임은 길이+형식(한글/영문/숫자, `NicknamePolicy`) 둘 다 검증하지만 본인 기존 닉네임과 실제로 다를 때만 — DTO의 `@Pattern`이 아니라 `UserService.updateMyProfile()`이 "바뀔 때만" 검사(아래 참고). **(2026-08-14 추가)** `interestRegion`은 DTO의 `@Size(max = 30)`으로 값이 있으면 항상 길이만 검증(형식 제약은 없음). 그 외 필드는 형식 검증 없음 |
| 닉네임 중복 확인 | ✅ (변경 시에만) |
...
8. **(2026-08-04, `deleteReplacedObject`로 완화됨)** confirm/reset 시 이전 S3 이미지 삭제는 여전히 best-effort지만, 즉시 삭제 전에 `status=pending`으로 다시 태깅해두는 안전망이 추가됨 — 즉시 삭제가 실패해도 버킷 Lifecycle 규칙(만료 기간 1일, AWS 콘솔에서 설정 확인함)이 최대 1일 내로 정리해줌. 태깅 자체와 즉시 삭제가 둘 다 실패하는 이중 실패 케이스만 여전히 영구 고아로 남을 수 있음 — 발생 확률이 낮아 별도 정리 배치까지는 아직 논의 안 됨. **(2026-08-24 확인)** 이 이중 실패 케이스를 완전히 닫을 수 있는 방법이 이미 이 저장소에 검증된 패턴으로 존재함 — `property` 도메인의 `PropertyImageOrphanCleanupJob`(`property/batch`)이 태그/Lifecycle에 의존하지 않고, S3의 `property-images/` prefix 전체 객체와 DB에 실제로 참조된 imageUrl 집합을 직접 대조해 그레이스 기간이 지난 미참조 객체를 정리한다. 같은 방식을 `profile-images/` prefix에 대해 `UserRepository.findAllProfileImageUrls()`(신규 필요) 기반으로 미러링하면, 태깅과 즉시삭제가 둘 다 실패하는 케이스까지 포함해 모든 고아 객체가 결국 정리된다 — Lifecycle 규칙(태그 기반)과는 독립적으로 동작하기 때문. 아직 구현하지는 않음 — 도입할지는 팀 판단 필요(현재는 발생 확률이 낮아 보류 중인 상태였으므로, 배치 하나를 새로 유지보수하는 비용 대비 실익이 있는지 확인 후 결정)
...
1. `UserPreference.interestRegion`에는 애플리케이션 레벨 길이 검증이 전혀 없다. `ProfileRegisterRequest.interestRegion`(`@NotBlank`만 있고 `@Size` 없음, `dto/ProfileRegisterRequest.java:18-19`)과 `ProfileUpdateRequest.interestRegion`(제약 없음, `dto/ProfileUpdateRequest.java:16`) 모두 닉네임과 달리 길이 제약이 없고, 엔티티 컬럼(`entity/UserPreference.java:30`)도 `@Column(length=...)` 지정 없이 Hibernate 기본값(VARCHAR(255))으로 생성된다. 실제 프런트(`ProfileClient.tsx`)는 3단계 select로만 값을 구성해 실무상 짧은 문자열만 만들어지지만, API를 직접 호출하면(Swagger/Postman 등) 255자를 넘는 문자열도 검증 없이 그대로 받아들여져 저장 시점에 DB 컬럼 길이 초과로 500(DataException)이 날 수 있다 — 닉네임처럼 400으로 정리되게 막히는 다른 실패 케이스들과 일관성이 없다. `interestRegion`에도 합리적인 `@Size(max = ...)`를 추가하는 것을 권장. **(2026-08-14 해결됨)** 프론트 `regions_nested.ts`(법정동 전체 데이터, 19,841개 조합)를 직접 파싱해 "시도 시군구 읍면동" 조합의 실측 최댓값이 20자(`전북특별자치도 전주시 완산구 남노송동` 등)임을 확인하고, 여유를 둔 `@Size(max = 30)`을 `ProfileRegisterRequest`/`ProfileUpdateRequest` 양쪽에, `@Column(length = 30)`을 `UserPreference.interestRegion`에 추가했다.
2. **(신규 발견, 팀 자체 테스트가 이미 증명하고 있음)** `User.changeRole()`/`suspend()`/`activate()`의 "탈퇴한 사용자는 대상에서 제외" 가드(`entity/User.java:116-139`)는 해당 트랜잭션이 그 행을 읽은 시점의 메모리 스냅샷을 기준으로 판단하기 때문에, 사용자 본인의 탈퇴 커밋과 경쟁하면 TOCTOU로 우회될 수 있다. `UserWithdrawFieldOverwriteIntegrationTest.concurrentRoleChangeDoesNotResurrectFieldsAnonymizedByWithdraw`(`src/test/java/com/algogyeyak/user/service/UserWithdrawFieldOverwriteIntegrationTest.java`)가 정확히 이 경쟁 상황(관리자의 `changeRole` 트랜잭션이 탈퇴 전 상태를 먼저 읽고, 탈퇴가 커밋된 뒤에 `changeRole`이 커밋)을 재현하는데, 이 테스트의 목적은 "PII 필드가 되살아나지 않는다"를 확인하는 것이지만 마지막 assertion에서 `assertThat(persisted.getRole()).isEqualTo(Role.ADMIN)`을 그대로 "정상 동작"으로 단언한다. 즉 결과적으로 **WITHDRAWN 상태이면서 Role=ADMIN인 계정**이 DB에 남을 수 있다는 사실을 테스트 스스로 증명하면서도, 그 함의는 별도로 짚어지지 않은 것으로 보인다. `changeNickname()`이 동일한 성격의 레이스에 대해 REQUIRES_NEW 재확인(최신 커밋 상태 재조회)을 두는 것과 달리, `changeRole`/`suspend`/`activate`에는 그런 안전망이 없다. 코드 주석의 판단은 "탈퇴 계정은 로그인 자체가 안 되니 role이 잘못 남아도 무해하다"는 것인데, 위 "회원 탈퇴" 표에서 확인했듯 `CustomOAuth2UserService.rejectIfBlocked()`가 탈퇴/정지 계정의 OAuth 재로그인을 이미 차단하고 있어 이 경로로 ADMIN 권한이 악용될 가능성은 낮다고 판단됨 — 다만 로컬 로그인/refresh 경로까지 전부 재확인한 것은 아니라 auth 담당자와 함께 한 번 더 확인 권장. **(2026-08-14 부분 해결됨)** `AdminUserService.updateRole()`/`updateStatus()`가 `User.changeRole()`/`suspend()`/`activate()`로 엔티티 필드를 바꾸고 커밋 시점 dirty-checking에 맡기던 방식을, `UserRepository.updateRoleIfNotWithdrawn()`/`updateStatusIfNotWithdrawn()` 조건부 UPDATE(`UPDATE ... WHERE id = ? AND status <> WITHDRAWN`)로 교체했다 — 락을 잡는 쓰기문이라 `findAllByRoleAndStatusForUpdate`와 동일하게 실행 시점의 최신 커밋 데이터로 WHERE를 평가하므로, 영향받은 row가 0건이면 "그 사이 탈퇴함"으로 안전하게 판단해 에러로 변환한다. `UserWithdrawFieldOverwriteIntegrationTest`가 재현하는 정확한 레이스(관리자가 먼저 읽고 탈퇴가 먼저 커밋된 뒤 관리자 쪽이 나중에 커밋)는 이제 막힌다. 다만 이건 완전 봉쇄는 아니다 — 관리자 쪽 UPDATE가 실제로 탈퇴보다 먼저 커밋되는 정상적인 순서에서는 여전히 WITHDRAWN+ADMIN 조합이 나올 수 있다(그 자체는 레이스가 아니라 실제 커밋 순서를 그대로 반영한 결과). `User.withdraw()`가 자신의 UPDATE에서 role도 함께 강제로 되돌리는 2단계(완전 봉쇄)와 DB `CHECK` 제약(코드 경로 밖의 실수까지 막는 최후 안전망)은 아직 적용하지 않았다 — 실제 로그인 차단으로 위험이 낮다고 판단해 보류 중. `UserWithdrawFieldOverwriteIntegrationTest` 자체는 `AdminUserService`를 거치지 않고 엔티티를 직접 조작해 `@DynamicUpdate`의 PII 보호 효과만 검증하는 별개 테스트라 이번 수정과 무관하게 그대로 유효하다. 회귀 테스트는 `AdminUserServiceTest.updateRoleThrowsWhenTargetWasConcurrentlyWithdrawn`/`updateStatusThrowsWhenTargetWasConcurrentlyWithdrawn`에 추가했다.
