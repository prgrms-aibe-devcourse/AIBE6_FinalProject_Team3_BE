...
- **(#233 완료)** `GET /properties`에 `hasSignal`(Boolean, 선택) 필터 추가 — 홈 화면 "위험 신호 확인" 카드가 신호 매물이 여러 건일 때 걸러볼 방법이 없어 전체 목록으로만 보내던 문제(`fix/home-signal-card`, FE PR #180)를 해소하기 위함. 위험신호 개수는 risk-analysis가 유지하는 데이터라 DB 쿼리 조건으로 바로 못 걸기 때문에, `PropertyService.getMyProperties()`가 `PropertyRiskSummaryProvider.getSummariesByUserId()` 호출을 페이지네이션보다 앞으로 당겨서(이 호출 자체는 필터 여부와 무관하게 원래도 매번 하던 것이라 새 비용은 아님) `hasSignal=true`일 때 신호 있는 매물 id 목록을 먼저 구하고, 그 목록을 `PropertyRepository.search()`의 새 파라미터 `signalPropertyIds`로 넘겨 기존 필터들과 동일한 `(:param IS NULL OR ...)` 패턴으로 필터링한다. 신호 있는 매물이 하나도 없으면 DB 조회 자체를 생략하고 빈 페이지를 바로 반환. 컬렉션 파라미터에 대한 null 체크가 다른 스칼라 조건들과 동일하게 동작하는지는 `PropertyRepositoryTest`에 `@DataJpaTest` 기반 실제 쿼리 테스트로 별도 확인함
...
7. ~~매물 목록 조회에 지역/면적/거래유형/주택유형/가격범위 검색이 전혀 없음~~ → `PropertySearchCondition`(region/minArea·maxArea/transactionType/propertyType/minDeposit·maxDeposit/minMonthlyRent·maxMonthlyRent) 추가로 해소됨(페이지네이션은 아래 7-1로 이미 분리 해소돼 있었음). ~~건물명(title) 검색은 아직 없음~~ **(2026-08-21 해소, 5차 멘토링 피드백 6-3)** `PropertySearchCondition.title`(부분일치 LIKE, region과 동일 패턴) 추가 — `GET /properties?title=` 쿼리파라미터로 노출. title 컬럼 자체는 NOT NULL(`resolveTitle()`이 비어있으면 매물유형 한글 라벨로 대체 저장)이라 이 조건에서 null 매물을 걱정할 필요는 없음
