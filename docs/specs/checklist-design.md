...
- **`GET /checklists`(내 체크리스트 목록)** — 본인 매물 전체와 매물별 체크리스트 진행 상태(시작 전 포함)를 한 번에 보여주기 위해 추가됨.
  ...
  - **(2026-08-21)** 응답에 `generalMissingCount`(미체크 일반 항목 수)/`requiredMissingCount`(미체크 필수 항목 수) 필드 추가 — 실제 목록 화면에서 "완료" 배지와 낮은 `progressPercent`(예: 24%)가 나란히 뜨는 모순이 발견됨. 원인은 `Checklist.refreshStatus()`의 COMPLETED 판정이 `REQUIRED` 항목만 보고 `GENERAL`은 무시하는데, `progressPercent`는 `REQUIRED`+`GENERAL` 전체 대비 체크된 비율이라 둘의 기준이 달랐기 때문. FE와 협의해 완료된 카드는 퍼센트바 대신 "일반 항목 N개 남음" 텍스트로 교체하기로 결정 — `generalMissingCount`가 이 텍스트의 근거값. `requiredMissingCount`는 반대로 진행중 카드의 "필수 항목 N개 남음" 표시용(COMPLETED면 정의상 항상 0). 둘 다 `generalMissingCount`가 먼저 추가된 뒤, 대칭성을 위해 `requiredMissingCount`가 같은 패턴(`findProgressByUserId()`에 `CASE WHEN` 집계 한 줄 추가)으로 뒤따라 추가됨 — 완료 액션 버튼(FE 로컬 계산, BE 액션 엔드포인트 없음)의 활성화 기준도 100%(전체)에서 REQUIRED로 낮추기로 함께 결정됐으나 이건 FE 전용 변경.
