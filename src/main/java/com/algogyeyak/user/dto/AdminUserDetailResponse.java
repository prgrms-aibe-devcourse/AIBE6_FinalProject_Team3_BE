...
public record AdminUserDetailResponse(
    ...
    // 새 role 값을 파라미터로 직접 받는다.
    public static AdminUserDetailResponse from(User user, Role role) {
        return new AdminUserDetailResponse(
                user.getId(), user.getEmail(), user.getNickname(), user.getProfileImageUrl(),
                role, user.getStatus(), user.getCreatedAt(), user.getUpdatedAt());
    }

    // from(User, Role)과 동일한 이유로, updateStatus()가 조건부 UPDATE로 status를 바꾼 뒤 쓴다.
    public static AdminUserDetailResponse from(User user, UserStatus status) {
        return new AdminUserDetailResponse(
                user.getId(), user.getEmail(), user.getNickname(), user.getProfileImageUrl(),
                user.getRole(), status, user.getCreatedAt(), user.getUpdatedAt());
    }
}
