 ...
 * seniorDeposit/maxClaimAmount(선순위보증금 반영 정밀 재계산 관련 값)는 아직
 * checkAndSave()가 항상 null로만 저장하므로(POST /deposit-safety/recalculate 미구현) 응답에
 * 넣지 않는다 - recalculate 구현 시 함께 추가한다.
 */
public record DepositSafetyCheckResponse(
    ...
    public static DepositSafetyCheckResponse from(Long propertyId, DepositSafetyCheck check) {
        if (check == null) {
            return new DepositSafetyCheckResponse(propertyId, null, null, false, null, null, null, null, DISCLAIMER);
        }
