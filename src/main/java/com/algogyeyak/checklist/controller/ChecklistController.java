...
public class ChecklistController {
    ...
    /**
     * 유저-매물 조합의 체크리스트를 문항까지 포함해 조회한다. 없으면 404를 반환한다 (생성은 POST가 담당).
     */
    @GetMapping("/properties/{propertyId}/checklists")
    public ApiResponse<ChecklistResponse> getChecklist(
            @AuthenticationPrincipal JwtUserPrincipal userDetails,
            @PathVariable Long propertyId
    ) {
        Checklist checklist = checklistService.getChecklist(userDetails.userId(), propertyId);
        return ApiResponse.success(ChecklistResponse.from(checklist));
    }
