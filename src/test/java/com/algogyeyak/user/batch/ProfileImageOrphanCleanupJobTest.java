...
/**
 * ProfileImageOrphanCleanupJob은 S3PresignService.deleteReplacedObject()의 태깅+즉시삭제가
 * 둘 다 실패해 태그도 없이 영구 고아로 남는 프로필 이미지를 정리한다. PropertyImageOrphanCleanupJob과
 * 동일한 네 가지 핵심 판단을 검증한다: 참조 없고 그레이스 기간이 지났으면 삭제 / 그레이스 기간
 * 이내면 보존 / DB에 참조돼 있으면 그레이스 기간과 무관하게 보존 / 삭제 후보 비율이 임계치를
 * 넘으면 서킷브레이커가 개입해 아무것도 삭제하지 않음.
 */
@ExtendWith(MockitoExtension.class)
class ProfileImageOrphanCleanupJobTest {
    ...
    private ProfileImageOrphanCleanupJob job;
    ...
    @BeforeEach
    void setUp() {
        job = new ProfileImageOrphanCleanupJob(
                s3PresignService, userRepository,
                new ProfileImageCleanupProperties(GRACE_PERIOD_HOURS, MAX_DELETE_RATIO)
        );
    }

    @Test
    void 참조가_없고_그레이스_기간이_지난_객체는_삭제한다() {
        String orphanKey = "profile-images/1/orphan.jpg";
        Instant old = Instant.now().minus(GRACE_PERIOD_HOURS + 1, ChronoUnit.HOURS);
        when(userRepository.findAllProfileImageUrls()).thenReturn(List.of());
        when(s3PresignService.listObjects(S3ImagePurpose.PROFILE))
    ...
    void 그레이스_기간_이내_객체는_보존한다() {
        String recentKey = "profile-images/1/recent.jpg";
    ...
    void DB에_참조된_객체는_그레이스_기간이_지나도_보존한다() {
        String referencedKey = "profile-images/1/used.jpg";
        String imageUrl = "https://example-bucket.s3.ap-northeast-2.amazonaws.com/" + referencedKey;
        Instant old = Instant.now().minus(GRACE_PERIOD_HOURS + 1, ChronoUnit.HOURS);
        when(userRepository.findAllProfileImageUrls()).thenReturn(List.of(imageUrl));
    ...
    void 삭제_후보_비율이_임계치를_넘으면_아무것도_삭제하지_않는다() {
        ...
        // 상황을 재현한다. 서킷브레이커의 표본 하한(MIN_OBJECTS_FOR_CIRCUIT_BREAKER=10)을 넘도록
        // 객체를 충분히 둔다.
        double strictRatio = 0.5;
        ProfileImageOrphanCleanupJob strictJob = new ProfileImageOrphanCleanupJob(
                s3PresignService, userRepository,
                new ProfileImageCleanupProperties(GRACE_PERIOD_HOURS, strictRatio)
        );
        Instant old = Instant.now().minus(GRACE_PERIOD_HOURS + 1, ChronoUnit.HOURS);
        List<S3ObjectSummary> allOrphaned = IntStream.range(0, 12)
                .mapToObj(i -> new S3ObjectSummary("profile-images/1/orphan-" + i + ".jpg", old))
                .toList();
        when(userRepository.findAllProfileImageUrls()).thenReturn(List.of());
        when(s3PresignService.listObjects(S3ImagePurpose.PROFILE)).thenReturn(allOrphaned);
    ...
    void 표본이_적으면_삭제_비율이_높아도_서킷브레이커가_개입하지_않는다() {
        ...
        List<S3ObjectSummary> fewOrphaned = List.of(
                new S3ObjectSummary("profile-images/1/a.jpg", old),
                new S3ObjectSummary("profile-images/1/b.jpg", old)
        );
        when(userRepository.findAllProfileImageUrls()).thenReturn(List.of());
        when(s3PresignService.listObjects(S3ImagePurpose.PROFILE)).thenReturn(fewOrphaned);

        strictJob.cleanUp();

        verify(s3PresignService).deleteObject("profile-images/1/a.jpg");
        verify(s3PresignService).deleteObject("profile-images/1/b.jpg");
    }
}
